...
export function renderThemeTokensJson(): string {
  // A COMPLETE, valid `SublimeTokens` object — the library reads `color.light`/
  // `color.dark`, `radii`, `shadows`, `spacing`, and `typography`, so the scaffold
  // must ship that exact shape (a partial object crashes `generateThemes` at
  // runtime). Edit the values; keep the shape.
  const light = {
...
/**
 * Typed app design tokens. Edit tokens.json; the explicit \`SublimeTokens\` type
 * makes tsc reject a tokens.json whose shape drifts from what the library expects.
 */
export const tokens: SublimeTokens = tokensJson;
`;
}
